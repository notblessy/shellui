//! Horizontal stack widget with SwiftUI-style API.
...
/// A horizontal stack container that distributes its children horizontally.
    ...
    Renderer: crate::core::renderer::Renderer,
{
    ...
    fn layout(
        ...
        for (i, child) in self.children.iter_mut().enumerate() {
            ...
            positioned_node.bounds = Rectangle::new(x, padding, size.width, size.height);
...
impl<'a, Message, Renderer> From<HStack<'a, Message, Renderer>> for Element<'a, Message, Renderer>
where
    Message: 'a,
    Renderer: crate::core::renderer::Renderer + 'a,
{
    fn from(stack: HStack<'a, Message, Renderer>) -> Self {
        Self::new(stack)
    }
}
