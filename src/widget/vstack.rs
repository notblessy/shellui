    ...
    Renderer: crate::core::renderer::Renderer,
{
    ...
    fn layout(
        ...
        let total_width = cross_size.min(limits.max_width).max(limits.min_width);
        let total_height = main_size.min(limits.max_height).max(limits.min_height);
        ...
        let positioned: Vec<Node> = child_nodes
            ...
            .map(|mut node| {
                ...
                let y = main_cursor;
                main_cursor += mh + self.spacing;
                node.bounds = Rectangle::new(cross_offset, y, mw, mh);
                node
            })
            .collect();
